...
// List UNPAIRED articles eligible for re-pairing (grouped display helper).
export async function listUnpaired(equipmentTypeId?: number) {
  let q = db.selectFrom('article as a')
    .innerJoin('equipment_type as et', 'et.equipment_type_id', 'a.equipment_type_id')
    .select(['a.article_id', 'a.barcode', 'a.equipment_type_id', 'et.name as equipment_type_name'])
    .where('a.state', '=', 'UNPAIRED');
  if (equipmentTypeId) q = q.where('a.equipment_type_id', '=', equipmentTypeId);
  return q.orderBy('et.name').orderBy('a.barcode').execute();
}
